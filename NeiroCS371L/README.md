...
| UI | Colors, Buttons, Navigation, General On-App Design | Final | Final | None. | Ethan (90%) Jacob (10%) | 
| Group "Jam" Feature | Allows users to connect with other users through group codes to create shared playlists based on multiple emojis | Final | Final | Note: After a host denies a particular user in a public group, that user cannot be accepted for the current group in the future, even if they request again. | Andy (80%) Allison (20%) | 
...
| Settings | Features dark mode/light mode toggle, excluded and preferred artists and genres selection, playlist length selection, sign out button | Alpha | Beta | Preferred genres/artists was added after a suggestion was made on the grading feedback. Dark mode/light mode, excluded artists/genres, and the playlist length were all included in the Alpha release, but preferred genres/artists were not included until the beta release. | Ethan (80%) Jacob (20%) | 
| LLM Integration | Incorporates Gemini LLM to allow users to create or update playlists based on natural language input | Final | Final | Requires Gemini API Key - see comment below. | Allison (80%) Jacob (20%) |
| Playlist History Screen | Allow users to see their past playlists, saved on Firestore| Beta | Beta | None | Jacob (60%) Andy (40%) | 
| Open-Source Playlist Generation | Utilized APIs from ListenBrainz and LastFM to make more mood-tailored playlists, and converted to Spotify IDs for easy integration | Final | Final | Originally thought we could use Spotify API for everything, but pivoted because Spotify Search was returning song titles with our keywords instead of songs with that mood | Jacob (100%) | 
